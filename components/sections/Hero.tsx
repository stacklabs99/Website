...
function Particles() {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 90 }, () => ({
      ...
      a: Math.random() * 0.28 + 0.06, ph: Math.random() * Math.PI * 2,
    }))
    const MAX_DIST = 120; let t = 0
    const draw = () => {
      ctx.clearRect(0, 0, c.width, c.height); t += 0.005
      for (const p of pts) {
        p.x += p.vx; p.y += p.vy
        if (p.x < -4) p.x = c.width + 4; if (p.x > c.width + 4) p.x = -4
        if (p.y < -4) p.y = c.height + 4; if (p.y > c.height + 4) p.y = -4
        const alpha = p.a * (0.4 + 0.6 * Math.sin(t + p.ph))
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(110,162,255,${alpha})`; ctx.fill()
      }
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (dist < MAX_DIST) {
            ...
            ctx.strokeStyle = `rgba(79,142,247,${(1 - dist / MAX_DIST) * 0.12})`
...
function MagBtn({ children, href, primary }: { children: React.ReactNode; href: string; primary?: boolean }) {
  ...
  return (
    <motion.a ref={ref} href={href}
      ...
      whileTap={{ scale: 0.95 }}
      className={`relative inline-flex items-center gap-2 px-7 py-4 rounded-[14px] text-[14px] font-semibold tracking-tight btn-shimmer
...
/* ─── Word Cycle ─────────────────────────────────────────────── */
const WORDS = ['Nível Mundial.', 'Inigualável.', 'Excecional.', 'Elite.']
function WordCycle() {
  ...
  return (
    <span className="relative inline-block" style={{ minWidth: '10ch' }}>
      <AnimatePresence mode="wait">
        <motion.span key={i} className="inline-block text-gradient-blue"
...
function Dashboard() {
  const metrics = [
    ...
    { v: '92.4K', l: 'Utilizadores', d: '+31%', c: '#8b5cf6' },
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ duration: 1.3, delay: 1.5, ease: [0.16, 1, 0.3, 1] }} className="relative">
      ...
      <motion.div animate={{ y: [0, -9, 0] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
        ...
        style={{ background: 'linear-gradient(145deg,rgba(10,10,22,0.98),rgba(6,6,16,0.99))', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 40px 100px rgba(0,0,0,0.85),inset 0 1px 0 rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/05">
          <div className="flex gap-1.5">
            {['#ff5f57','#ffbd2e','#28c840'].map(c => <div key={c} className="w-2.5 h-2.5 rounded-full opacity-70" style={{ background: c }} />)}
          </div>
          <div className="flex-1 flex justify-center">
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-md bg-white/04">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-[10px] text-white/25 font-mono">app.cliente.io/dashboard</span>
            </div>
          </div>
        </div>
        <div className="p-5 space-y-3.5">
          <div className="grid grid-cols-3 gap-2.5">
            {metrics.map((m, idx) => (
              <motion.div key={m.l} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 1.7 + idx * 0.08 }} className="p-3 rounded-xl border border-white/05" style={{ background: `${m.c}09` }}>
          ...
          <div className="rounded-xl border border-white/04 p-3.5 bg-white/01">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-semibold text-white/60">Crescimento de Receita</span>
              <span className="text-[10px] text-white/22">12 meses</span>
            </div>
            <div className="flex items-end gap-1 h-[52px]">
              {bars.map((h, idx) => (
                <motion.div key={idx} className="flex-1 rounded-t-sm"
                  initial={{ scaleY: 0 }} animate={{ scaleY: 1 }}
                  transition={{ delay: 1.85 + idx * 0.04, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                  style={{ height: `${h}%`, transformOrigin: 'bottom', background: idx >= 10 ? 'linear-gradient(to top,#4f8ef7,#8b5cf6)' : `rgba(79,142,247,${0.1 + (idx / 11) * 0.25})` }} />
              ))}
            </div>
          </div>
          <div className="space-y-1.5">
            {[
              { color: '#10b981', text: 'v4.2.1 lançado em produção', t: '2m' },
              { color: '#4f8ef7', text: 'Contrato enterprise assinado', t: '1h' },
            ].map((a, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 2.15 + idx * 0.12 }}
      ...
      <motion.div initial={{ opacity: 0, x: -20, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }}
        ...
        style={{ background: 'rgba(8,8,20,0.95)', boxShadow: '0 20px 50px rgba(0,0,0,0.65)', backdropFilter: 'blur(24px)' }}>
        <div className="text-[9px] text-white/28 uppercase tracking-[0.14em] mb-1.5">Latência</div>
        ...
        <div className="mt-2.5 h-1 rounded-full bg-white/06 overflow-hidden">
          <motion.div className="h-full rounded-full bg-emerald-500" initial={{ width: 0 }} animate={{ width: '94%' }}
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[10px] text-white/40 font-medium">Lançado</span>
        </div>
        <div className="text-xs text-white/60 font-medium">Produção ativa</div>
        <div className="text-[10px] text-white/22 mt-0.5">Agora mesmo</div>
      </motion.div>
    </motion.div>
  )
}
...
export default function Hero() {
  ...
  const orb1x = useTransform(sx, [0,1], [-55, 55]); const orb1y = useTransform(sy, [0,1], [-38, 38])
  const orb2x = useTransform(sx, [0,1], [38, -38]); const orb2y = useTransform(sy, [0,1], [28, -28])
  const cardX = useTransform(sx, [0,1], [16, -16]);  const cardY = useTransform(sy, [0,1], [12, -12])
  const spotLeft = useTransform(sx, [0,1], ['5%', '95%'])
  const spotTop  = useTransform(sy, [0,1], ['5%', '95%'])

  useEffect(() => {
    const fn = (e: MouseEvent) => { mx.set(e.clientX / window.innerWidth); my.set(e.clientY / window.innerHeight) }
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[100dvh] flex items-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 110% 70% at 50% -5%,rgba(79,142,247,0.09) 0%,transparent 55%)' }} />
        <div className="absolute inset-0 grid-pattern opacity-[0.28]" />
        <Particles />
        <motion.div className="absolute rounded-full pointer-events-none"
          style={{ width: 700, height: 700, left: spotLeft, top: spotTop, x: '-50%', y: '-50%', background: 'radial-gradient(circle,rgba(79,142,247,0.075) 0%,transparent 68%)' }} />
        <motion.div className="absolute rounded-full"
          style={{ x: orb1x, y: orb1y, width: 900, height: 900, top: '-20%', left: '-18%', background: 'radial-gradient(circle,rgba(79,142,247,0.14) 0%,transparent 62%)' }}
          animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute rounded-full"
          style={{ x: orb2x, y: orb2y, width: 750, height: 750, bottom: '-25%', right: '-12%', background: 'radial-gradient(circle,rgba(139,92,246,0.12) 0%,transparent 62%)' }}
          animate={{ opacity: [0.5, 0.9, 0.5] }} transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut', delay: 4 }} />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 80% 80% at 50% 50%,transparent 40%,rgba(5,5,7,0.55) 100%)' }} />
        ...
        <div className="grid lg:grid-cols-[1fr_460px] xl:grid-cols-[1fr_500px] gap-12 xl:gap-20 items-center">
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }}
              ...
              <span className="text-[11px] text-white/40 tracking-wide">Q1 2026 — 3 vagas disponíveis</span>
              <span className="text-[11px] font-semibold text-[#4f8ef7]">Candidatar →</span>
            </motion.div>

            {/* Headline */}
            <h1 className="font-display font-bold mb-7" style={{ lineHeight: 0.91, letterSpacing: '-0.04em' }}>
              {[
                { text: 'Engenharia de',   color: 'rgba(255,255,255,1)',    delay: 0.38 },
                { text: 'Produtos',        color: 'rgba(255,255,255,0.18)', delay: 0.54 },
                { text: 'Digitais a',      color: 'rgba(255,255,255,1)',    delay: 0.70 },
              ].map(({ text, color, delay }) => (
                <span key={text} className="block overflow-hidden pb-1">
                  <motion.span className="block" style={{ fontSize: 'clamp(3.4rem,6.8vw,7.2rem)', color }}
                    initial={{ y: '108%' }} animate={{ y: 0 }}
                    transition={{ duration: 1.1, delay, ease: [0.16, 1, 0.3, 1] }}>{text}</motion.span>
                </span>
              ))}
              <span className="block overflow-hidden pb-1">
                <motion.span className="block" style={{ fontSize: 'clamp(3.4rem,6.8vw,7.2rem)' }}
                  initial={{ y: '108%' }} animate={{ y: 0 }}
            ...
            {/* Sub */}
            <motion.p className="text-white/35 text-[1.05rem] leading-relaxed max-w-[420px] mb-10"
              initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.9, delay: 1.15, ease: [0.16, 1, 0.3, 1] }}>
              Desenhamos e desenvolvemos os produtos digitais que definem empresas ambiciosas.
              Estratégia, craft e execução — obsessivamente unidos.
            </motion.p>
            ...
            <motion.div className="flex items-center gap-3 mb-14"
              ...
              <MagBtn primary href="#contact">Iniciar Projeto →</MagBtn>
              <MagBtn href="#projects">Ver Trabalho</MagBtn>
            </motion.div>
            ...
            <motion.div className="flex items-center gap-8"
              ...
              {[
                { v: 120, suffix: '+', l: 'Projetos entregues', delay: 2000 },
                { v: 98,  suffix: '%', l: 'Retenção de clientes', delay: 2200 },
                { v: 4,   suffix: '×', l: 'ROI médio', delay: 2400 },
              ].map(s => (
                <div key={s.l}>
      ...
      {/* Scroll indicator */}
      <motion.div className="absolute bottom-7 left-1/2 -translate-x-1/2 hidden md:flex flex-col items-center gap-2"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.2 }}>
        <motion.div className="w-[18px] h-7 rounded-full border border-white/12 flex items-start justify-center pt-1.5"
